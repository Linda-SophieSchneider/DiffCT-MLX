...
@_FASTMATH_DECORATOR
def _cone_3d_footprint_backward_sparse_kernel(
    d_sino, n_views, n_u, n_v,
    d_out, n_samples, d_indices, Nx, Ny, Nz,
    du, dv, d_src, d_det_center, d_det_u, d_det_v, cx, cy, cz, voxel_spacing
):
    """Sparse adjoint: evaluate only the voxels listed in ``d_indices``.

    ``d_indices[isample]`` is a flat C-order index into a (D, H, W) = (Nz, Ny, Nx)
    volume; the result is written to ``d_out[isample]`` (a 1D vector). Thread =
    one requested voxel. Same footprint weights as the dense backward kernel.
    """
    isample = cuda.grid(1)
    if isample >= n_samples:
        return

    flat_idx = d_indices[isample]
    ix = flat_idx % Nx
    iy = (flat_idx // Nx) % Ny
    iz = flat_idx // (Nx * Ny)
    ...
    for iview in range(n_views):
        ...
        qy = src_y + t * ry
        qz = src_z + t * rz
        u0 = (qx - det_cx) * u_vec_x + (qy - det_cy) * u_vec_y + (qz - det_cz) * u_vec_z
        v0 = (qx - det_cx) * v_vec_x + (qy - det_cy) * v_vec_y + (qz - det_cz) * v_vec_z
